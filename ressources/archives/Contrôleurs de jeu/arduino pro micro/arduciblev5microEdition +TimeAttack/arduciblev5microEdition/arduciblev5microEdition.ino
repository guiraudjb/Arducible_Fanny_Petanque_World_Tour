...
void UsbPCGameControler(){
  ...
  lcd.setCursor(0,3);
  lcd.print(F("To run normal game"));
while (digitalRead(boutonG) || digitalRead(boutonD)){
    ...
    if(digitalRead(cible1)) {
       Keyboard.press('e');  
    pixels.setPixelColor(0, pixels.Color(0, 0, intensiteLed));
    pixels.show();   
    } else {
      Keyboard.release('e');
    ...
    if(digitalRead(cible2)) {
        Keyboard.press('r');
        pixels.setPixelColor(1, pixels.Color(0, 0, intensiteLed));
    pixels.show();   
    } else {
        Keyboard.release('r');
    ...
    if(digitalRead(cible3)) {
        Keyboard.press('t');
        pixels.setPixelColor(2, pixels.Color(0, 0, intensiteLed));
    pixels.show();   
    } else {
        Keyboard.release('t');
...
void AffichageTimer(){
   lcd.clear();
   lcd.setCursor(0,0);
   lcd.print(F("TIME LEFT   SCORE"));

   String timeleft = String(GameTimer);
   if (timeleft.length()==3){
      String centaine = String(timeleft.charAt(0));
      printBigNum(centaine.toInt(), 0, 2);
      String decimale = String(timeleft.charAt(1));
      printBigNum(decimale.toInt(), 3, 2);
      String unite = String(timeleft.charAt(2));
      printBigNum(unite.toInt(), 6, 2);
   }
   else if (timeleft.length()==2){
      String decimale = String(timeleft.charAt(0));
      printBigNum(decimale.toInt(), 1, 2);
      String unite = String(timeleft.charAt(1));
      printBigNum(unite.toInt(), 4, 2);
   }
   else{
      String unite = String(timeleft.charAt(0));
      printBigNum(unite.toInt(), 3, 2);
    }

   String PlayerScore = String(Score);
   if (PlayerScore.length()==3){
      String centaine = String(PlayerScore.charAt(0));
      printBigNum(centaine.toInt(), 11, 2);
      String decimale = String(PlayerScore.charAt(1));
      printBigNum(decimale.toInt(), 14, 2);
      String unite = String(PlayerScore.charAt(2));
      printBigNum(unite.toInt(), 17, 2);
   }
   
   else if (PlayerScore.length()==2){
      String decimale = String(PlayerScore.charAt(0));
      printBigNum(decimale.toInt(), 12, 2);
      String unite = String(PlayerScore.charAt(1));
      printBigNum(unite.toInt(), 15, 2);
   }
   else{
      String unite = String(PlayerScore.charAt(0));
      printBigNum(unite.toInt(), 14, 2);
    }
